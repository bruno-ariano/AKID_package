...
for i in k_deter_code:
	print ('Preparing kinase %d of %d\r'%(list(k_deter_code.keys()).index(i)+1,len(k_deter_code))),;sys.stdout.flush()
...
for i in okk:
	print ("processing Kinase %d"%(list(okk.keys()).index(i)+1) +" of %d (%s)"%(len(okk),i))
